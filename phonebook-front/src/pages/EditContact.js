  ...
  export default function EditContact( ) {
    ...
    useEffect(() => {
      fetchContact();
      console.log(formData.emails.length);
      console.log(formData.phone_numbers.length);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [id]); // Fetch contact whenever the ID changes
    ...
    async function fetchContact() {
      try {
        ...
        // Update the form state with the fetched contact data
        setFormData({
          ...
          emails: contactData.emails.map(email => email.email),
          phone_numbers: contactData.phone_numbers.map(number => number.phone_number)
        });
    
        // Update the position buttons based on the lengths of emails and phone_numbers
        setEmailPositionButton(contactData.emails.length > 1);
        setNumberPositionButton(contactData.phone_numbers.length > 1);
      } catch (error) {
    ...
    const handleAddField = field => {
      setFormData(prevFormData => {
        ...
        else if (field === "phone_numbers" && newFormData[field].length > 1)
          setNumberPositionButton(true);
        return newFormData;
      });
    };
    ...
    const handleRemoveField = (index, field) => {
      setFormData(prevFormData => {
        ...
        if (field === "emails" && newFormData[field].length === 1)
          setEmailPositionButton(false);
        else if (field === "phone_numbers" && newFormData[field].length === 1)
          setNumberPositionButton(false);
        return newFormData;
      });
    };
    ...
    const handleSubmit = async e => {
      ...
      try {
        // Send form data to API for updating the contact
        await api.put(`/contacts/${id}`, formData);
        console.log("Contact updated successfully");
        // Navigate to the PhoneBook page after successful update
        navigate("/");
      } catch (error) {
      ...
      <div className="flex flex-col justify-center items-center">
        <div className="flex flex-col w-2/4 m-9">
          <div className="flex justify-start w-full mb-10">
            {" "}
            <h2 className="text-3xl font-bold mb-4">Edit contact</h2>
          </div>
